...
function StationIllustration({ batteryPercent, chargingStatus, solarWatts, phoneConnected }: Props) {
  ...
  const svgW = Math.min(SCREEN_W - 72, 320);
  const scale = svgW / 320;
  ...
  return (
    <View style={styles.container}>
        ...
        <Animated.View style={[styles.sun, { transform: [{ rotate: spin }] }]}>
          <Svg width={40} height={40}>
            ...
            {[0, 45, 90, 135, 180, 225, 270, 315].map((a) => {
              ...
              return (
                <Line key={a} x1={20 + Math.cos(rad) * 10} y1={20 + Math.sin(rad) * 10}
      ...
      <Svg width={svgW} height={180 * scale} viewBox="0 0 320 180">
        <Defs>
...
const styles = StyleSheet.create({
  container: { alignItems: "center", justifyContent: "center", minHeight: 160 },
  sun: { position: "absolute", top: 2, right: 20, zIndex: 1 },
  ledGlow: {
